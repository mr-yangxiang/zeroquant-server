...
2026-09-13 兼容调整（迁移 008，须先完成独立备份及上线授权）：

- 用户、价格历史、旧分析记录的 ID 统一为文本，保留已有数字标识和 UUID 值；只为新记录生成 UUID。
- 唯一性同时检查约束及有效的全表普通唯一索引；部分、表达式、非唯一和无效索引不能冒充唯一约束，INCLUDE 列不作为唯一键。
- 显式登记旧库已有的辅助字段，放宽文本长度时不截断已有内容。
- 两张旧预测表的无时区 created_at 仅作为明确的兼容例外，输出 LEGACY_COLUMN_TYPE 警告；不转换历史值、不用于认证历史数据可用时间。
- 无错误不代表没有风险：仍需检查 warnings，不能把保留的历史时区警告当作已修复。
- 服务启动会自动调用迁移执行器。008 未获批准前，不要发布该迁移到线上自动启动路径。
